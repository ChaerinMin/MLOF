...
def flow_warp(x, flow12, pad='border', mode='bilinear',args=None):
    B, _, H, W = x.size()
    ...
    if 'align_corners' in inspect.getfullargspec(torch.nn.functional.grid_sample).args:
        if args.first_order or args is None:
...
def get_occu_mask_bidirection(flow12, flow21, scale=0.01, bias=0.5,args=None):
    flow21_warped = flow_warp(flow21, flow12, pad='zeros',args=args)
...
def get_occu_mask_backward(flow21, th=0.2):
    ...
    return occu_mask.float(), corr_map